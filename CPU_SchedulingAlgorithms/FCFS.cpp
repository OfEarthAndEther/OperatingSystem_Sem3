...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        if (time_passed < processes[i].arrival) {
            time_passed = processes[i].arrival;  // CPU waits for next process
        }

        processes[i].start = time_passed;
        ...
        time_passed = processes[i].finish;
    }
